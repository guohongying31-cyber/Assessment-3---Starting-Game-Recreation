...
| 2 / Feature-Visual | `art: import cultivator directional and death frames`; `art: add four Shan Hai creature sprite sets`; `art: add pickups life icon and six base wall sprites`; `anim: configure cultivator and power pellet controllers`; `anim: configure ghost state cycles and scene showcase` | Required frames and directions, visible scene examples, all ten states for every beast, correct controller names and preview durations |
...
Keep asset settings and relevant production files available for future editing.
Record checks as they run, and distinguish imported assets from completed gameplay.
This plan is not evidence of a completed submission.
